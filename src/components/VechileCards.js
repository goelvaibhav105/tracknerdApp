...
export default function VechileCards({ userData,setDetailPage,setVechileClicked }) {
    console.log(userData,"userData")
    const setVechileFunc = (vechile) => {
        setVechileClicked(vechile)
        setDetailPage(true)
    } 
    if(!userData){
    ...
    return (
        <section className="text-gray-600 body-font">
                    <div className="container px-5 py-2 mx-auto">
                        ...
                        {userData && userData[0].vehicles && userData[0].vehicles.map((vechile,index) => {
                return (
                            <div key={index} className="p-4 lg:w-1/3 cursor-pointer" onClick={() =>setVechileFunc(vechile)}>
